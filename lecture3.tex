...
Введем обозначение $_{[t]}a = a$, если t = и, иначе = $\neg$a \\
Тогда $_{[\llbracket a \rrbracket]}$a, $_{[\llbracket b \rrbracket]}$b $\vdash$ $_{[\llbracket a \star b \rrbracket]}$a $\star$ b \\
\\
Пусть a содержит $p_1, .. p_n$ Докажем, что \\
$_{[\llbracket p_1 \rrbracket]}p_1, .. _{[\llbracket p_n \rrbracket]}p_n \vdash _{[\llbracket a \rrbracket]}a$ \\
Доказательство по индукции \\
База: $a = p, p \vdash p, \neg p \vdash \neg p$\\
Переход: Обозначим $_{[\llbracket p_1 \rrbracket]}p_1, .. _{[\llbracket p_n \rrbracket]}p_n$ - $\Pi$ \\
a = b $\star$ c. Тогда по предположению $\Pi$ $\vdash$ $_{[\llbracket b \rrbracket]}$b и $\Pi$ $\vdash$ $_{[\llbracket c \rrbracket]}$c \\
Тогда по нашему обозначению $_{[\llbracket b \rrbracket)}$]b, $_{[\llbracket c \rrbracket]}$c $\vdash$ $_{[\llbracket a \rrbracket]}$a \\
\\
Теперь если a общезначимо, то $_{[\llbracket p_1 \rrbracket]}p_1, .. _{[\llbracket p_n \rrbracket]}p_n \vdash a$ \\
в частности имеем $_{[\llbracket p_1 \rrbracket]}p_1, .. _{[\llbracket p_{n-1}  \rrbracket]}p_{n-1},p_n \vdash a$ и \\
$_{[\llbracket p_1 \rrbracket]}p_1, .. _{[\llbracket p_{n-1}  \rrbracket]}p_{n-1}, \neg p_n \vdash a$ откуда по лемме можно выкинуть\\
$p_n$. после $n-1$ итерации получим $\vdash$ a чтд
...
База k = 0. Тогда перед нами аксиома, необходимо проверить, что все аксиомы общезначимы. Проведем разбор несколько из них, остальное будет аналогично. Для проверки общезначимости необходимо убедиться в справдливости формулы сделав непосредственную подстановку. \\
Сх.1 a $\to$ b $\to$ a \\
\\
