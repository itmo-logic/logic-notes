...
Следовательно, так как оценка корректна, а оценка имликации $\alpha \rightarrow \beta$ задается функцией $f_{\rightarrow}(\llbracket \alpha \rrbracket, \llbracket \beta \rrbracket)$, то $\llbracket P_i  \rightarrow  P_j \rrbracket = \texttt{И}$ (достаточно рассмотреть формулу $ \alphav \rightarrow \alpha$).
