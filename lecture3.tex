...
\begin{lemma}\label{any_connection}
$_{[\llbracket \alpha \rrbracket]} \alpha, _{[\llbracket \beta \rrbracket]} \beta \vdash _{[\llbracket \alpha \star \beta \rrbracket]} \alpha \star \beta$, где $\star$ --- любая связка.
\end{lemma}
...
\begin{lemma}
Если $\alpha$ содержит $n$ пропозициональных переменных $P_1 \dots P_n$, то
\[_{[\llbracket P_1 \rrbracket]}P_1, \dots _{[\llbracket P_n \rrbracket]}P_n \vdash _{[\llbracket \alpha \rrbracket]}\alpha\]
\end{lemma}

\begin{proof}
Докажем индукцией по длине формулы.

База: $\alpha = P$. Тогда очевидно, что $P \vdash P$, $\neg P \vdash \neg P$.

Переход: Пусть утверждение уже доказано для любых формул, короче некоторой формулы $\alpha$. Докажем его для формулы $\alpha$.

Обозначим $_{[\llbracket P_1 \rrbracket]}P_1, \dots _{[\llbracket P_n \rrbracket]}P_n$ за $\Pi$.

$\alpha \equiv \beta \star \gamma$. Тогда, так как формулы $\beta$ и $\gamma$ короче формулы $\alpha$, то к ним применимо предположение индукции, то есть $\Pi$ $\vdash$ $_{[\llbracket \beta \rrbracket]}\beta$ и $\Pi$ $\vdash$ $_{[\llbracket \gamma \rrbracket]}\gamma$.
По лемме \ref{any_connection}, $_{[\llbracket \beta \rrbracket]}\beta, _{[\llbracket \gamma \rrbracket]}\gamma \vdash _{[\llbracket \alpha \rrbracket]}\alpha$.
\end{proof}

\begin{lemma}\label{derivability_with_propositionals}
Если $\models \alpha$ и $\alpha$ содержит $n$ пропозициональных переменных $P_1 \dots P_n$, то $_{[x_1]} P_1, \dots _{[x_n]} P_n \vdash \alpha$ при любых $x_i$.
\end{lemma}

\begin{proof}Частный случай леммы \ref{derivability_with_propositionals} и определения $\models$.\end{proof}
...
$x_1, \dots x_n$. Тогда по лемме \ref{derivability_with_propositionals}
$_{[x_1]} P_1, \dots _{[x_{n}]} P_{n} \vdash \alpha$.

Переход. Пусть утверждение уже доказано для некоторого $n-k \geq 0$, покажем его
для $n-k+1$ (то есть доказано для $k \leq n$, покажем его для $k-1$).
...
База. Пусть $k=0$. Тогда перед нами аксиома. Докажем, что все аксиомы общезначимы. Для проверки общезначимости необходимо сделать непосредственную подстановку и убедиться в истинности формулы. Проведем разбор для двух аксиом --- остальные будут аналогичны.
...
$\alpha$ & $\beta$ & $\neg\beta$ & $\neg\alpha$ & $\alpha \to \beta$ & $\alpha \to \neg\beta$ & $(\alpha \to \neg\beta) \to \neg\alpha$ & $(\alpha \to \beta) \to (\alpha \to \neg\beta) \to \neg\alpha$ \\
\hline
Л & Л & И & И & И & И & И & И\\
И & Л & И & Л & Л & И & Л & И\\
Л & И & Л & И & И & И & И & И\\
...
Таким образом, чтобы доказать, что некоторая формула не выводима в 
...
\begin{center} \tikz{
\node at (0,0)   (W1) {$W_1$};
\node at (4,0) (W2) {$W_2 \Vdash P, P \vee\neg P$};
\draw[->] (W1) to (W2); 
}\end{center}
...
\begin{center} \tikz{
\node at (0,0) (W1) {$W_1 \Vdash P$};
\node at (4,0) (W2) {$W_2 \Vdash P, Q$};
\draw[->] (W1) to (W2); 
}\end{center}
...
\begin{center} \tikz{
\node at (0,0) (W1) {$W_1$};
\node at (3,2) (W2) {$W_2 \Vdash P$};
\node at (3,0) (W3) {$W_3 \Vdash Q$};
\node at (3,-2) (W4) {$W_4 \Vdash R$};
\draw[->] (W1) to (W2); 
\draw[->] (W1) to (W3); 
\draw[->] (W1) to (W4); 
}\end{center}
...
\begin{definition}Назовем пропозициональное исчисление \emph{табличным}, если для этого исчисления 
