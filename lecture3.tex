...
из данных импликаций: $P \rightarrow Q$,  $Q \rightarrow R$ , $R \rightarrow P$. П
окажем, например, что $P \rightarrow Q$ не является истинной в $W_1$. 
...
в этом исчислении), и она может быть представлена конечной таблицей. То есть существуют $n \in \mathbb{R}$,
...
достаточно будет показать, что предложенная выше формула истинна в этой модели, но недоказуема в ИИВ.
\begin{enumerate}
...
Исходная формула не вынуждается в корне $W_1$, так как, чтобы она была истинна в данном мире, необходима, чтобы 
хотя бы одна из импликаций была истинна в данном мире. Однако, во всех мирах $W_i$: $W_1 \preceq W_i$ вынуждена
лишь одна переменная $P_{i-1}$, тем самым нельзя показать, что какая-либо импликация вида $P_{i-1} \rightarrow P_j$ (где $i \ne j$)  вынуждена в $W_1$. Таким образом, $\phi_n$ недоказуема в интуиционистском исчислении высказываний.
\end{enumerate}
 Для произвольной табличной модели мы построили формулу $\phi_n$, которая является истинной в данной модели, но не является доказуемой, тем сам никакая табличная модель ИИВ не является полной.
\end{proof}